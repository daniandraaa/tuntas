...
<body style="background-color: #333353;">
    ...
    <div class="pembungkus px-2">
        ...
        @foreach ($users->where('status', '') as $user)
            <div class="container px-5" id="belumtuntas" style="background-color: #008189; border-radius: 50px; margin-top: 15px; align-items: center; justify-content: center; text-align: center; display:flex; justify-content: space-between;">
                    <h1 style="color: white; height: calc(9rem + ((1vw - 7.68px) * 0.6944)); align-items: center; justify-content: center; display: flex">{{ $user->nama_lengkap }} {{ $user->kelas }}</h1>
                    <div class="">
                        <form action="/tolak/{{ $user->id }}" method="POST">
                            @csrf
                            <button type="submit" name="submit" class="btn btn-danger" style="display: ">TOLAK</button>
                        </form>
                        <form action="/terima/{{ $user->id }}" method="POST">
                            @csrf
                            <button type="submit" name="submit" class="btn" style="background-color: #12DD00">TERIMA</button>
                        </form>
                    </div>
            </div>
        @endforeach
